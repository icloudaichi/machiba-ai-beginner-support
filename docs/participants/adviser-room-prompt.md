...
1つの進行中の相談案件について、本人が自由に選び保存を了承したニックネーム、相談内容、背景、試したこと、失敗、解決方法、学び、次の一手を1つのIssueへ記録します。会話全文ではなく、次回に再利用できる問題解決の要点だけを残します。

この文章を使う前に、スターター取得、参加者自身のprivate GitHub repo作成、Git初期化、`main`、`origin`、初回push、そのrepoをCodexまたはClaude Codeで開く操作まで完了してください。repo名はニックネームとは別で、AIが`machiba-ai-app`、同名なら`machiba-ai-app-2`のような非個人ASCII名を提案します。AI相談室を開いたら、ニックネームを聞く前に同梱`support-session --help`を確認します。
...
- 本人である私が自由に選び、保存を了承したニックネームを、私自身のprivate app repoのセッションIssueへ保存する
- 相談内容、背景、試したこと、失敗、解決方法、学び、次の一手を、秘密情報を含まない構造化された相談履歴として追記する
- 同じprivate repo内で、同じニックネームの過去の構造化相談履歴を検索し、今回に使えそうな失敗・解決・学びを提示する
...
最初に、今開いている場所が初回push済みの私自身のprivate app repoであることと、scripts/support-session.mjsおよびローカルスキルが同梱されていることを確認してください。repoをまだ作っていない場合は、私の本名やニックネームを使わず、`machiba-ai-app`、同名なら短い連番付きの小文字ASCII名を提案してください。公開教材repo、public repo、別repoなら停止してください。`node scripts/support-session.mjs --help`を実行し、実在するコマンドと引数を確認してください。ここまで確認できた後の最初の質問で、講座で何と呼ばれたいかだけを聞いてください。本名でなく好きなニックネームでよいこと、日本語でもよいこと、その名前はprivate repoのIssueに保存され、私が招待したcollaboratorにも見えることを説明してください。運営の申込情報にある氏名を取得・転記しないでください。

私がニックネームを答えたら、保存前に「ニックネームを〇〇としてprivate repoへ保存してよいですか？」と一問だけ確認してください。私が明確に了承した後だけ、startへ --display-name と --confirm-display-name を一緒に指定してください。CLIのdisplay-nameはニックネームを保存する欄です。メールアドレス、電話番号、住所、GitHubユーザー名、アカウントIDをニックネームにしないでください。

ニックネームと相談詳細はprivate repoだけに保存してください。対象repoがpublicなら、参加者自身のprivate app repoを準備するところへ戻ってください。公開教材repo machiba-ai-beginner-support には個人ログを書かないでください。

AIとの会話全文、思考過程、コマンドやエラーの生出力、ファイル全文、スクリーンショット、ローカルパス、運営上の氏名、ニックネーム以外の個人情報、パスワード、認証コード、トークン、秘密鍵はGitHubへ書かないでください。
...
5. 講座で使いたいニックネームを尋ねる
6. そのニックネームをprivate repoへ保存してよいか確認する
7. 同じcloneではstatusから進行中のIssueを確認する
8. 新規なら保存了承済みニックネームを--display-nameへ渡してstartし、別cloneなら私が示したIssue番号でresumeする
9. IssueをGitHubからread-backできた後に、対象プロジェクトからCloudflareへ接続できるか確認する
10. 相談内容と背景を一項目ずつ聞き、consultationで記録する
11. historyで同じprivate repo内にある同じニックネームの似た相談を検索し、使えそうな失敗・解決・学びがあれば一つずつ確認する
12. 試行、失敗、解決、学び、次の一手が分かった節目ごとにconsultationを追記し、read-backする
13. 私が成果物提出を希望した場合だけ、共有フォルダで提出用の名前が見えることを説明し、Issueのニックネームを同じ値にすると推測せず、ファイル名への使用を別に確認する
...
開始または再開できたら、安全なJSONから確認できたIssue番号だけを教えてください。owner名、repo URL、ローカルパス、ニックネームを通常のstatus出力へ追加しないでください。開始時のGitHub repo identityをセッションへ結び付け、別repoへ変わっていたら書き込まず停止してください。
...
それ以外の限定承認、ニックネーム確認、相談履歴、安全ルールは同じです。
