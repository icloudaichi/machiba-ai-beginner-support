...
◎ ニックネーム付き相談と成果物提出をpublic repoで常に拒否すると、誤公開をfail closedにできる。
...
❌ private repoを作るために英字のニックネームまで考えてもらうと、アプリ制作前の不要な判断が増える。

判断：AIが`machiba-ai-app`、同名なら短い連番付きの非個人ASCII repo名を提案する。講座で呼ばれたいニックネームは日本語でもよく、repo名とは分離する。

### BALTHASAR

◎ 運営上の氏名を閲覧制限した申込情報・運営台帳に残し、private Issueでは本人が選んだニックネームを使うと、個別フォローと本人の選択を両立できる。

判断：運営台帳だけで氏名、ニックネーム、private repoを対応させ、氏名をIssueやrepo名へ転記しない。

### MELCHIOR

❌ Issue用ニックネーム、repo名、Drive提出名を同じ値として扱うと、異なる公開範囲へ本人が意図しない名前が流れる。

判断：4つの値を別状態として扱う。Drive提出名は共有範囲を説明して別に了承を得る。参加者IDへの移行はこの変更に含めず、必要時に別設計とする。
...
決定：公開範囲を先に確認する。仮説Bではニックネーム付きの自動相談履歴を止め、参加者自身のprivate repoを準備する。
