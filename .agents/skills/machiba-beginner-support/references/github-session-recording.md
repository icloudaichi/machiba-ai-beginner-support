...
- 表示名を含む詳細な相談履歴と成果物提出記録はprivate repoだけで扱い、公開repoでは作成・検索しない。
...
- 本人が確認した表示名でセッションIssueを1件作成、または同じ相談案件のIssueを再開する。
...
Issue本文はセッションの識別、本人確認済み表示名、開始時刻、目的、記録方針を保持する。private repoのIssueタイトルは`AI相談｜表示名｜日付`とし、表示名を公開Issueへ出さない。現在地の履歴は相談コメントへ追加し、本文を毎回書き換えない。秘密情報を含まない短い値だけを使う。
...
表示名は、本人が保存を明確に了承した値だけを使う。本名は必須ではない。OS名は必要な場合だけ`macOS`または`Windows`と記録してよい。端末名、GitHubユーザー名、メールアドレス、ローカルパスは記録しない。
...
`--folder-id`は指定値に完全一致させる。`--parent-verified`は、Driveからファイルの親とファイル名の両方を確認できた場合だけ付ける。`--drive-url`には個別ファイルURLだけを渡す。提出先フォルダURL、認証情報付きURL、推測URLを使わない。個別URLを確認できない場合は引数を省略し、Issue上で`未記録`と分かる状態を残す。Issue本文には生のフォルダIDを残さず、`街場のAI屋さん・当日成果物フォルダ`と親確認済み状態を記録する。`artifact`はconfirmed display nameを持つprivateセッションだけで使う。
...
表示名付きセッションを開始または再開した後、現在の相談を短い検索語へ要約して履歴を確認できる。

```bash
node scripts/support-session.mjs history --query "Wrangler 接続" --limit "5"
```

履歴検索は、現在のprivate repoで同じ表示名markerを持つ過去セッションのうち、CLIが生成した相談コメントだけを安全なJSONへ変換する。Issue本文や一般コメントの全文を返さない。返される項目はIssue番号、記録日時、相談内容、背景、試したこと、失敗、解決方法、学び、次の一手に限る。
...
2. 記録内容をallowlistの項目だけで組み立て、本人確認済み表示名以外の個人情報、秘密、生出力がないことを確認する。
...
表示名付きの標準セッションはprivate repoでだけ再開する。公開repoのIssueを標準相談履歴として読み書きしない。
