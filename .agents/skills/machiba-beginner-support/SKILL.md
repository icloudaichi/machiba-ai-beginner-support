...
description: 対象プロジェクトを開いたCodexまたはClaude Codeで、本人が選び保存を了承したニックネームに紐づく相談・失敗・解決・学び・成果物提出をprivate repoの1相談案件1 Issueへ記録し、過去履歴を再利用しながら初心者を伴走する。普通のAIチャットでは自動記録しない。
---
...
1. PCからGitHubへ接続できるか、安全な読み取り確認をする。接続済みなら、アカウント作成や認証をやり直さない。
2. スターター取得済みで、参加者自身のprivate GitHub repoとしてGit初期化、`main`、`origin`、初回pushまで確認済みかを確かめる。repoをまだ作っていない場合は、ニックネームと分離した非個人のASCII名`machiba-ai-app`を提案し、同名があれば末尾へ短い連番を付ける。未完了なら、CloudflareやIssueへ進まず一つずつ準備する。
3. CodexまたはClaude Codeが、そのprivate repoのフォルダを開いていることを確認する。公開教材repoや別repoなら停止する。
4. `scripts/support-session.mjs`とローカルスキルが同梱されているか確認し、`node scripts/support-session.mjs --help`を実行する。helpにないコマンドや引数を推測しない。
5. サイトの端末内進捗にニックネームがあれば、同じ値を候補として示し、聞き直さない。なければ講座で使いたいニックネームだけを尋ねる。本名は不要で日本語でもよい。運営の申込情報にある氏名を取得・転記しない。
6. そのニックネームがprivate repoのIssueとcollaboratorにも見えることを説明し、端末内進捗への保存とは別に、Issueへ保存してよいか明確に確認する。
7. 限定承認を確認し、`status`で進行中の相談を確認する。なければ保存了承済みニックネームを`--display-name`へ渡してIssueを開始し、別cloneで状態を失った場合だけ明示されたIssue番号から再開する。書き込み後はGitHubからread-backする。
8. Issueをread-backできた後で、対象プロジェクトからCloudflareへ接続できるか安全に確認する。未接続なら一度に一操作だけ案内する。
9. 相談内容と背景を一項目ずつ確認し、過去の同じニックネームの相談履歴に似た失敗・解決がないか調べる。
10. 接続中に見えた具体的な操作から案内方法を提案し、相談・制作を同じスレッドで進める。
11. 成果物提出を希望した場合だけ、共有範囲と提出用の名前を別に確認し、ブラウザからGoogle Driveへ提出する。Issueのニックネームをそのまま流用しない。本人が希望し、時間と環境が許す場合は、Google公式OAuth／公式コネクタによるAI提出を発展として一操作ずつ案内できる。
...
本人が選び保存を了承したニックネーム、相談内容、背景、試したこと、失敗、解決方法、学び、次の一手を、分かった時点で構造化して継続記録する。過去の失敗と解決を、同じ本人の次の相談で再利用できる「相談履歴」にする。

会話全文、思考過程、コマンドの生出力、ファイル全文、ローカルパス、運営上の氏名、メールアドレス、認証コード、アカウントID、秘密情報は記録しない。保存了承済みニックネーム以外の個人情報を保存しない。

名前の区分とrepo名を決めるときは[名前と保存先](references/naming-and-identity.md)を読む。記録項目、ニックネーム確認、履歴再利用は[相談履歴](references/consultation-history.md)を読む。限定承認、read-back、commit連携は[GitHubセッション記録](references/github-session-recording.md)を読む。成果物を提出するときだけ[Google Driveへの成果物提出](references/drive-submission.md)を読む。

- 標準開始プロンプトの限定承認は、保存了承済みニックネーム、安全な相談履歴、別途承認済みでread-backできた成果物提出結果を、参加者自身のprivate app repoにある1つのIssueへ作成・再開・追記・read-backし、目的完了時にcloseすることを継続して許可する。
...
- ニックネームを含む相談履歴は公開リポジトリへ作らない。対象がpublicならprivate repoの準備へ戻る。
...
- ニックネーム付きで開始：`node scripts/support-session.mjs start --goal "目的" --display-name "保存了承済みニックネーム" --confirm-display-name`
...
ニックネーム付き`start`、`consultation`、`artifact`、`history`、ニックネーム付きセッションの`resume`はprivate repoだけで使う。公開repoでは標準の相談記録を開始・再開しない。
...
- サイトがこの端末の進捗へ保存するニックネームと、private Issueへの保存了承を分ける。Issueには用途と閲覧範囲を説明して明確な了承を得た後だけ保存する。日本語でもよい。運営上の氏名、メールアドレス、GitHubユーザー名、アカウントIDをニックネームとして取得・転記しない。
- repo名はニックネームと分離し、AIが`machiba-ai-app`または短い連番付きの非個人ASCII名を提案する。参加者へ英字のニックネームを考えさせない。
- パスワード、認証コード、トークン、秘密鍵、カード情報、保存了承済みニックネーム以外の個人情報を要求・保存・復唱しない。
...
- Drive提出用の名前は共有フォルダ内の参加者やリンク所持者にも見えると説明し、private Issue用ニックネームとは別に、同じ名前を使うか、ファイル名へ使用してよいか確認する。
