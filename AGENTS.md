# 街場のAI屋さん・公開サポートガイド

このリポジトリは、AI・GitHub・Cloudflareを初めて使う人のための公開教材とサポート資料です。CodexやClaude Codeが作業するときは、参加者が専門用語を知らない前提で案内してください。標準のAI相談室は、参加者の対象プロジェクトを開いたCodex／Claude Codeスレッドです。

## 最初に読むもの

1. `README.md`
2. `docs/README.md`
3. 初心者サポートでは `.agents/skills/machiba-beginner-support/SKILL.md`
4. 変更対象に関係する資料

参加者スターターを作る場合は、`docs/operations/starter-support-kit.md`を読み、記録ツール、Codexスキル、Claude Code入口を同じ版で同梱する。

## 初心者サポート

- GitHub・Cloudflareの接続確認から始め、長いレベル確認を先に行いません。
- GitHub接続後、参加者自身のprivate app repoに1つの進行中の相談案件につき1 Issueを開始・再開します。
- 同じcloneやworktreeでは`status`、別cloneや状態消失時は明示Issue番号の`resume --issue`を使い、似たIssueを推測しません。
- 成功、失敗、`blocked`、次の一手を構造化して記録し、書き込み後にread-backします。
- 一度に一問または一操作だけ案内します。
- 接続準備中に見えた具体的な操作を材料に、準備後の案内方法を調整します。
- 人を「初心者レベル1」のように採点しません。
- 標準開始プロンプトの限定承認は、セッションIssueの作成・追記・read-backと目的完了時のcloseだけに使います。
- 本人がコード変更を依頼した場合だけ、その依頼範囲を変更・検証・commit・pushし、remote SHAをIssueへ記録します。
- mergeとdeployは、別の明示依頼があるまで行いません。
- 普通のChatGPT・Claudeは説明専用とし、GitHubコネクタの有無にかかわらず、自動セッション記録を行いません。

## 表現

- 専門用語を使う前に、普段の日本語で役割を説明します。
- 「分からない」を失敗として扱わず、今見えている画面から次の一つを案内します。
- コピーできる文章、成功時に見えるもの、返答してほしいことを明確にします。
- 既に成功した操作をやり直させません。

## 安全

- パスワード、認証コード、トークン、秘密鍵、カード情報、個人情報を要求・保存・復唱しません。
- GitHub・Cloudflareの接続診断は、標準出力と標準エラーを捨て、終了コードだけで判定します。
- コマンド出力に含まれるユーザー名、メールアドレス、アカウントIDをAIのツール実行記録や回答へ残しません。
- 診断失敗時も、未加工の出力を取得せず、一段階ずつ原因を切り分けます。
- 認証や公開の確認を、別の外部操作の許可として扱いません。
- 会話全文、思考過程、コマンドの生出力、ローカルパス、ユーザー名、メールアドレス、アカウントIDをIssueへ書きません。
- 公開repoには、本人の明示許可と`--allow-public`なしでセッションを記録しません。
- セッションと公開許可を開始時のcanonical repo名・不変repo IDへ結び付け、repo不一致ではfail closedにします。
- 公開教材repoには参加者ログを書かず、教材改善Issueだけを受け付けます。
- 本物の顧客データを教材やサンプルへ入れません。
- 秘密情報を誤って公開した場合は、値を引用せず失効・再発行を案内します。

## ファイルを変更するとき

- 依頼された範囲を確認し、関係のない変更を元に戻しません。
- アプリ本体を変更する依頼と、初心者への助言を同じ作業として混ぜません。
- UIやコードを変更した場合は `npm test` を実行します。
- ドキュメントだけを変更した場合も、リンク、相対パス、秘密情報の混入を確認します。
- `scripts/support-session.mjs`の使い方を書く場合は、実際の`--help`と終了コードを確認し、存在しない引数を推測しません。
- 最後に変更内容、確認結果、残っていることを普段の日本語で報告します。
