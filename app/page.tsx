...
const practiceSteps = [
  {
    ...
    lead: "公式ZIPを展開するか、すでにPCにある展開済みスターターを使って、自分の作品にする準備をします。",
    actions: ["公式ZIPをダウンロードするか、PC内の展開済みスターターを選ぶ", "ZIPの場合は展開して普通のフォルダにする", "README.mdとscriptsフォルダがあることを確認する"],
    prompt: deckPrompt("starter-acquisition", "準備用チャット", `公式ZIPから始めるか、すでにPCにある展開済みスターターを使うかを最初に一つだけ確認してください。ZIPが必要な場合は ${STARTER_ZIP_URL} だけを使い、保存と展開を一操作ずつ案内してください。ローカルのフォルダがある場合は再ダウンロードせず、そのフォルダを使ってください。どちらの場合もREADME.mdとscripts/support-session.mjsが見えることを確認してください。まだCloudflare接続やIssue作成は始めず、フォルダの絶対パス、ユーザー名、個人情報はチャットへ書かせないでください。`),
